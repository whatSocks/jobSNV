...
RETURN g.name AS Group, s.name AS Skill, count(DISTINCT p) AS `Persons`
ORDER BY Persons DESC, g.name ASC 
----
//table
...
RETURN c.name AS Company, count(DISTINCT p) AS `Persons`, s.name AS `Skills Required`
LIMIT 10
----
//table
