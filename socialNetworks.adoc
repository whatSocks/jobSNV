...
CREATE (`1`)-[:KNOWS]->(`2`), (`1`)-[:KNOWS]->(`3`), (`1`)-[:KNOWS]->(`4`), (`1`)-[:KNOWS]->(`5`), 
	...
	(`3`)-[:KNOWS]->(`4`), (`3`)-[:KNOWS]->(`5`), (`1`)-[:KNOWS]->(`31`), 
	(`4`)-[:KNOWS]->(`5`), 
	(`5`)-[:KNOWS]->(`7`), 
	(`6`)-[:KNOWS]->(`7`), 
	(`8`)-[:KNOWS]->(`7`), 
...
What if a Company were less interested in social fit, how would they pick the best group advertise in?
...
First, find an expert. Let's define an expert as someone with lots people in his or her network with the same skills. 

Next, let's continue the search for a suitable Django developer. 
...
What groups are the better-connected experts in? 

[source,cypher]
----
MATCH (s:Skill)-[:HAS_SKILL]-(expert:Person)-[:KNOWS]-(a:Person)-[:HAS_SKILL]-(s),(expert)-[:MEMBER]-(g:Group)
WHERE s.name = 'Django'
RETURN DISTINCT g.name AS Group, s.name AS Skill, count(DISTINCT expert) AS `# Experts`
ORDER BY Skill DESC , `# Experts` DESC
----
//table
