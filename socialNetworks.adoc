...
- *user interestes*: (I search for 'butterflies' on Amazon.com while logged into Facebook and am then presented with lepidopterae-related ads accross Facebook)
- *social network*:  Companies on Facebook can target ads at the friends of particular users, as these users presumably share relevant characteristics with the initial users, such as location and interests

=== This GraphGist 

This GraphGist explores targeted 'marketing' on job networking sites such as LinkedIn, Vieadeo, and Xing using a combination of the techniques above. Our theoretical professional networking site allows users and companies to use the site for free, but does charge for  
